...
export function JobsPage() {
  ...
  // 批次与单条分成两个 tab：两者各自分页，挤在同一页里既不好扫，也没法各自触底加载
  const [listTab, setListTab] = useState<'batches' | 'jobs'>('batches')
  ...
  useEffect(() => {
    // 切到「单条」时批次卡片不在页面上，展开详情的轮询也跟着停
    if (!expandedBatchId || listTab !== 'batches') return
    let active = true
  ...
  // 空态只看当前 tab：另一个 tab 有几条跟这一屏无关
  const activeEmpty = listTab === 'batches' ? batches.length === 0 : jobs.length === 0

  return (
    <div className="page">
      <header className="page-header"><div><h1>任务队列</h1><p>本机正在处理和已经结束的提取任务。</p></div></header>
      <div className="toolbar">
        <div className="segmented-switch" role="tablist" aria-label="任务类型">
          <button
            type="button"
            role="tab"
            aria-selected={listTab === 'batches'}
            className={listTab === 'batches' ? 'active' : ''}
          ...
          <button
            type="button"
            role="tab"
            aria-selected={listTab === 'jobs'}
            className={listTab === 'jobs' ? 'active' : ''}
      ...
      <section className="panel jobs-page-panel">
        {activeEmpty ? (
          <EmptyState
            icon={ListTodo}
            title={statusFilter === 'all' ? '队列空闲' : '这个状态下没有任务'}
          ...
          {batches.map((batch) => {
            ...
            // 右列的操作按钮组只在真有按钮时渲染：已完成的批次若没有失败项，右列就只剩状态芯片。
            // 条件逐项对齐下面四个按钮各自的出现条件，别漏掉 paused 这种「未结束但不算 active」的组合
            ...
            return (
              <article className="batch-card" key={batch.id}>
                <div className="batch-card-head">
                  ...
                  >
                    ...
                    <span className="icon-swap" aria-hidden="true">
                      <ChevronUp className={isExpanded ? undefined : 'is-off'} size={18} />
                      <ChevronDown className={isExpanded ? 'is-off' : undefined} size={18} />
                    </span>
                  </button>
                  <div className="batch-title">
                    ...
                    <small>
                      ...
                      {` · 创建于 ${createdAtFormatter.format(new Date(batch.created_at))}`}
                    </small>
                  </div>
                  {/* 右侧一行：操作按钮在左、状态芯片在右。芯片压在最右，
                      和任务卡片、右栏批次列表里「状态芯片收尾」的排法保持一致 */}
                ...
                {/* 进度条只在批次还没结束时才有意义：跑完的批次停在 100%，
                    既不提供信息，又和上面「共 N 条 · 已完成 N」的摘要重复 */}
                {active && (
                        ...
                        >
                          ...
                          <div className="batch-child-main">
                            ...
                            <div className="batch-child-detail">
                              ...
                              {/* 平台徽标跟着时间走：标题行留给作品名，两行都塞满时读起来更清爽 */}
          ...
          {batchCursor && (
            <LoadMore label="加载更多批次" loading={loadingMore === 'batches'} onLoad={() => void loadMore('batches')} />
          )}
        </> : <>
          {jobs.map((job) => {
          const title = jobDisplayTitle(job, titles)
          ...
          {jobCursor && (
            <LoadMore label="加载更多任务" loading={loadingMore === 'jobs'} onLoad={() => void loadMore('jobs')} />
          )}
        </>}
      </section>
      {previewId && (
