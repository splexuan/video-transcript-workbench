...
- **兜底解析**：本机解析失败时（多为平台风控），配了第三方聚合解析接口的 Key 就自动改用它取无水印直链继续识别；没配则保持原样报错，不会多发任何请求（详见「兜底解析接口」）。
...
- 视频号链接：本机没有可用的解析方案，配置兜底解析网关后可直接粘贴分享链接提取；未配置时请先自行下载视频后导入本地文件。
...
B站、抖音、快手、小红书都是**本机解析**，平台风控或页面结构变化时可能整条链路失败。这时如果配了第三方解析接口的 Key，工作台会改用它取直链继续识别。

**网关地址由使用者自行配置**（代码里不内置任何第三方服务地址）：

```powershell
$env:VTW_FALLBACK_API_BASE = "https://你的网关地址"
```

未配置时兜底链路完全不参与，行为与没有这个功能时一致。网关需要兼容下面这套约定（本项目按 BugPk-Api 风格的响应实现，其它同类网关只要能对上字段也能用）：
...
- **只在主链路失败后才使用**：没配 Key 时不发任何额外请求，错误提示与接入前完全一致。
...
- 换网关域名不用改代码：设 `VTW_FALLBACK_API_BASE` 环境变量即可。
